...
export function PluginCatalogScreen({
  ...
  const { palette, styles } = usePluginCatalogStyles();
  const resolvedCopy = {
    ...defaultCopy,
    ...copy,
            ...
            <View accessibilityViewIsModal style={styles.sheet}>
              ...
              <ScrollView contentContainerStyle={styles.sheetContent}>
                <View style={styles.detailTitle}>
                  ...
                  <View><Text style={styles.detailName}>{selected.name}</Text><Text style={statusStyle(selected, styles)}>{resolvedCopy.connectionStatusLabels[selected.connectionStatus] ?? selected.statusLabel}</Text></View>
                </View>
...
function AiAccessConnectionRow({
  ...
  const { styles } = usePluginCatalogStyles();
  return (
    <View style={[styles.row, styles.rowSeparator]}>
      <Pressable
  ...
  return (
    <View style={[styles.row, styles.rowSeparator]}>
      ...
      <View style={styles.trailingCell}>
        <Text numberOfLines={2} style={statusStyle(item, styles)}>{connectionStatusLabels[item.connectionStatus] ?? item.statusLabel}</Text>
        <PluginCatalogActionButton
          busyActionKey={busyActionKey}
          item={item}
          onAction={onAction}
...
function GenericConnectionIcon() {
  const { palette } = usePluginCatalogStyles();
...
function statusStyle(item: PluginCatalogItem, styles: ReturnType<typeof createPluginCatalogStyles>) {
...
function usePluginCatalogStyles() {
  const palette = useMobilePalette();
  return useMemo(() => ({ palette, styles: createPluginCatalogStyles(palette) }), [palette]);
}

function createPluginCatalogStyles(palette: MobilePalette) {
  ...
  rowMain: { alignItems: "center", flex: 1, flexDirection: "row", gap: 11, minWidth: 0 },
  ...
  trailingCell: { alignItems: "flex-end", flexShrink: 0, gap: 6 },
